...
Chat.prototype.addHTML = function() {
  $displayMessage.append(`
    <article id=${this.id} class="show-recent-message user1">
      ...
      <button class="delete-button" type='image'>Delete</button>
    </article>
    <article class="user2"><aside class="current-message">Totally.</aside>
    </article>
  `);
};
...
var ChatroomRepo = {
  ...
  showTenMessages: function() {
    for (var i = this.messageArray.length - 10; i < this.messageArray.length; i++) {
      this.renderMessageToPage(this.messageArray[i]);
      this.enableButton();
    }
  },
